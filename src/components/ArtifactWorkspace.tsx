...
export const ArtifactWorkspace: React.FC<ArtifactWorkspaceProps> = ({
  ...
  const { generateReflection } = useReflection();
  ...
  const handleTriggerReflection = async (artifactToUpdate: ArtifactItem) => {
    if (isGenerating) return;
    ...
    try {
      // Call the useReflection hook API under the hood
      const reflectionText = await generateReflection(
        currentStudent.englishName.split(" ")[0],
        activeSubject.name,
        artifactToUpdate.tags
      );

      if (reflectionText) {
        onUpdateArtifact(artifactToUpdate.id, { aiNarrative: reflectionText });
      } else {
        throw new Error("Failed to compile 2-sentence target reflection.");
      }
    } catch (err: any) {
      console.error("Reflection generation failure:", err);
      setNotesError(err.message || "Failed to compile concise reflection.");
      // Fallback
      const studentNameShort = currentStudent.englishName.split(" ")[0];
      onUpdateArtifact(artifactToUpdate.id, {
        aiNarrative: `${studentNameShort} demonstrated comprehensive conceptual grasp and positive development regarding ${artifactToUpdate.tags.slice(0, 2).join(" use and ")} during this study cycle. Moving forward, we will concentrate on refined language application drills to reinforce overall writing fluency and confidence.`
      });
    } finally {
      setIsGenerating(false);
      setGenId(null);
